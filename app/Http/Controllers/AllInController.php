...
class AllInController extends Controller
{
    ...
    public function listInvoice(Request $request){
        ...
        // }else{
            $fktr = Faktur::orderBy('id','desc')->where('attribute1',$id)->get();
            // $inv = Invoice::orderBy('id','desc')->where('attribute1',$id)->get();
        // }
        $data_arr = array();
        // dd($inv);
        foreach ($fktr as $record) {
            $data_arr[] = array(
                "id" => $record->faktur_id,
                "fktr_vendor" => $record->vendor->nama_vendor,
                "fktr_ppn" => number_format($record->ppn_fktr),
                "fktr_nilaitrx" => number_format($record->nilai_transaksi_fktr),
                "fktr_total" => number_format($record->total_fktr),

                "code_vendor" => $record->inv->vendor->no_id_vendor,
                "nama_vendor" => $record->inv->vendor->nama_vendor,
                "no_faktur" => $record->inv->no_faktur,
                "termin" => number_format($record->inv->termin_pembayaran),
                "trx" => number_format($record->inv->nilai_transaksi),
                "potongan_harga" => number_format($record->inv->potongan_harga),
                "ppn" => number_format($record->inv->ppn),
                "informasi" => $record->inv->informasi_pembayaran,
    ...
    public function listBupot(Request $request){
        ...
        foreach ($bupot as $record) {
            $data_arr[] = array(
                ...
                "created_by" => $record->users->name,
                "bruto" => number_format($record->jumlah_bruto),
    ...
    public function listPph(Request $request){
        ...
        foreach ($pph21 as $record) {
            $data_arr[] = array(
                ...
                "pph21perbulan" => number_format($record->pph21_perbulan),
                "status" => $record->attribute3,
                "created_by" => $record->users->name,
            );
        }
    ...
    public function listFiskal(Request $request){
        ...
        foreach ($fiskal as $record) {
            $data_arr[] = array(
                ...
                "nilai_debit" => number_format($record->nilai_debit),
                "nilai_kredit" => number_format($record->nilai_kredit),
    ...
    public function listPrepopulate(Request $request){
        ...
        foreach ($ppr as $record) {
            $data_arr[] = array(
                ...
                "jumlah_dpp" => number_format($record->jumlah_dpp),
                "jumlah_ppn" => number_format($record->jumlah_ppn),
    ...
    public function listBilling(Request $request){
        ...
        foreach ($billing as $record) {
            $data_arr[] = array(
                ...
                "npwp" => $record->npwp,
                "jenispajak" => $record->jenispajak->kode.'-'.$record->jenispajak->jenis_pajak,
                "jenis_setoran" => $record->jenissetoran->kode.'-'.$record->jenissetoran->jenis_setoran,
                "masapajak" => $record->masa_pajak,
                "masaaktif" => Carbon::parse($record->end_periode_pajak)->format('d-M-Y'),
                "jumlah" => number_format($record->jumlah),
                "created_by" => $record->users->name,
                "status" => $record->attribute3,
                "created_at" => $record->created_at->format('d-M-Y'),
            );
    ...
    public function listHutangppn(Request $request){
        ...
        foreach ($hutangppn as $record) {
            $data_arr[] = array(
                "id" => $record->id,
                "trx" =>$record->trx,
                "ppn_masuk" => number_format($record->jumlah_ppn_masuk),
                "ppn_keluar" => number_format($record->jumlah_ppn_keluar),
                "hutang_ppn" => number_format($record->hutang_ppn),
                "created_by" => $record->users->name,
                "status" => $record->attribute3,
                "created_at" => $record->created_at->format('d-M-Y'),
            );
    ...
    public function listPphtidakfinal(Request $request){
        ...
        foreach ($pphfinal as $record) {
            $data_arr[] = array(
                ...
                "kop" =>Str::limit($record->koptfinal->kode_objek.'-'.$record->koptfinal->penerima_penghasilan,35),
                "bruto" =>number_format($record->bruto),
                "tarif" =>number_format($record->hasil1+$record->hasil2+$record->hasil3+$record->hasil4),
